...
def create_mask(image):
    """
    create a binary coverage mask and distance transform from image
    :param image: 4-channel image
    :return: (binary mask with 1 where the image has content, 0 elsewhere; distance transform of that
        mask, giving each covered pixel's distance to the nearest uncovered/edge pixel)
    """
...
def max_levels(shape, requested):
    """
    Cap the pyramid depth so pyrDown/pyrUp never collapses a dimension to 0 or 1 px.
    """
    smallest_dim = min(shape[0], shape[1])
    possible = int(np.floor(np.log2(max(smallest_dim, 1))))
    return max(1, min(requested, possible))


def blend_images(image_a, image_b, levels, show=False):
    """
    Multi-band (Burt & Adelson) blend of 2 images using Laplacian pyramids.

    The seam mask is BINARY (1 => take from image_a, 0 => take from image_b), assigned per
    overlap pixel to whichever image's coverage extends further at that point (via each
    image's distance transform). Building a Gaussian pyramid from that binary mask, rather
    than from a continuous distance-ratio mask, is what makes this true multi-band blending:
    pyrDown's implicit blur widens the transition band progressively more at coarser
    (lower-frequency) pyramid levels while keeping it narrow at fine levels, which is what
    hides the seam without the ghosting/blurring a single continuous alpha-feather mask
    (effectively a Gaussian-blended blend) produces when the images aren't perfectly aligned.
    ...
    :param levels: the number of levels in the Laplacian pyramid (capped automatically for
        small/thin overlap regions)
    :return: 4-channel image that blends a and b
    """
    mask_a, distA = create_mask(image_a)
    mask_b, distB = create_mask(image_b)

    # binary seam: overlap pixels go to whichever image's coverage reaches further here;
    # pixels covered by only one image trivially resolve to that image since the other's
    # distance transform is 0 there
    mask = (distA >= distB).astype(np.float32)

    levels = max_levels(image_a.shape, levels)
    ...
    # alpha: pass through the source alpha where only one image covers a pixel, fully opaque
    # in the blended overlap, transparent where neither image covers it -- computed from the
    # actual coverage masks rather than guessed from which output pixels happen to be black
    only_a = (mask_a > 0) & (mask_b == 0)
    only_b = (mask_b > 0) & (mask_a == 0)
    overlap = (mask_a > 0) & (mask_b > 0)

    alpha = np.zeros(mask_a.shape, dtype=np.uint8)
    alpha[only_a] = image_a[:, :, 3][only_a]
    alpha[only_b] = image_b[:, :, 3][only_b]
    alpha[overlap] = 255
    img[:, :, 3] = alpha
